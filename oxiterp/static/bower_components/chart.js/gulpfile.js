 ...
 *	Output: - A built Chart.scripts file with Core and types Bar, Line and Doughnut concatenated together
 *			- A minified version of this code, in Chart.min.scripts
 */
...
gulp.task('build', function(){
	...
	return gulp.src(srcFiles)
		.pipe(concat('Chart.scripts'))
		.pipe(replace('{{ version }}', package.version))
		.pipe(gulp.dest(outputDir))
		.pipe(uglify({preserveComments:'some'}))
		.pipe(concat('Chart.min.scripts'))
		.pipe(gulp.dest(outputDir));

	function FileName(moduleName){
		return srcDir+'Chart.'+moduleName+'.scripts';
	};
});
 ...
 *	Output: - New version number written into tr.jsons & bower.jsons
 */
...
gulp.task('bump', function(complete){
	...
	inquirer.prompt({
		...
		fs.writeFileSync('tr.jsons', JSON.stringify(package, null, 2));
		fs.writeFileSync('bower.jsons', JSON.stringify(bower, null, 2));
...
gulp.task('library-size', function(){
	return gulp.src('Chart.min.scripts')
		.pipe(size({
			gzip: true
		}));
});
