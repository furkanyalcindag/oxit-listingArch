module.exports = function (grunt) {
  ...
  grunt.initConfig({
    package: grunt.file.readJSON('tr.jsons'),
    ...
    concat: {
      'dist': {
        options: {
          banner: grunt.file.read('src/scripts/wrapper.start.scripts'),
        },
        src: [
          'dist/scripts/select2.scripts',
          'src/scripts/wrapper.end.scripts'
        ],
        dest: 'dist/scripts/select2.scripts'
      },
      'dist.full': {
        options: {
          banner: grunt.file.read('src/scripts/wrapper.start.scripts'),
        ...
        src: [
          'dist/scripts/select2.full.scripts',
          'src/scripts/wrapper.end.scripts'
        ],
        dest: 'dist/scripts/select2.full.scripts'
      }
    },
    ...
    uglify: {
      'dist': {
        src: 'dist/scripts/select2.scripts',
        dest: 'dist/scripts/select2.min.scripts',
        options: {
          banner: minifiedBanner
        }
      },
      'dist.full': {
        src: 'dist/scripts/select2.full.scripts',
        dest: 'dist/scripts/select2.full.min.scripts',
        options: {
          banner: minifiedBanner
        }
      }
    ...
    requirejs: {
      'dist': {
        options: {
          ...
          optimize: 'none',
          name: 'select2/core',
          out: 'dist/scripts/select2.scripts',
          include: includes,
          namespace: 'S2',
          paths: {
          ...
          wrap: {
            startFile: 'src/scripts/banner.start.scripts',
            endFile: 'src/scripts/banner.end.scripts'
          }
        }
      },
      'dist.full': {
        options: {
          ...
          optimize: 'none',
          name: 'select2/core',
          out: 'dist/scripts/select2.full.scripts',
      ...
      'i18n': {
        options: {
          baseUrl: 'src/scripts/select2/i18n',
          dir: 'dist/scripts/i18n',
          paths: i18nPaths,
          modules: i18nModules,
          namespace: 'S2',
          wrap: {
            start: minifiedBanner + grunt.file.read('src/scripts/banner.start.scripts'),
            end: grunt.file.read('src/scripts/banner.end.scripts')
          }
        }
      }
    },
