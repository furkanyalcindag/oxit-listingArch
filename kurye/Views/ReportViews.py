...
from kurye.models.ReportRequestObject import ReportRequestObject
...
def request_report(request):
    ...
    for request1 in requests:
        ...
        active = TaskSituationTask.objects.filter(
            task__request__company_id=request1.pk).filter(
            Q(task_situation__name='Yolda') | Q(task_situation__name='Kurye Atandı')).filter(isActive=True).count()
